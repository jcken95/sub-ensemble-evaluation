# influenza admissions target script
...
deps_$need(
  "MRFtools",
  msg = "We need to install MRFtools from github directly, for now!",
  install_cmd = quote(devtools::install_github("eric-pedersen/MRFtools"))
)
...
config_file_path <- here::here("influenza", "models", "admissions", "influenza_admissions_config.yaml")
...
targets::tar_option_set(
  # note: worker settings are added to reduce memory usage on central node
  # *experimental*
  storage = "worker",
  retrieval = "worker",
...
model_targets <- tarchetypes::tar_map(
  ...
  targets::tar_target(
    ...
    command = {
      ...
      do.call(
        box::use,
        list(substitute(
          . / influenza / models / admissions / models / MODEL["fn" = FN],
          list(
  ...
  targets::tar_target(preds, model |> purrr::map("geo_preds") |> purrr::list_rbind())
)
...
list(
  ...
  targets::tar_target(
    config_file,
    here::here("influenza", "models", "admissions", "influenza_admissions_config.yaml"),
    format = "file"
  ),
  targets::tar_target(
    config,
  ...
  targets::tar_target(
    ...
    command = {
      ...
      rs$REDACTED |>
        dplyr::filter(date >= max(date) - 365L) |> # TODO make this better - based on max training length etc
        # keep only recent data and acute
        dplyr::select(
          date,
          population,
          trust_code,
          dplyr::starts_with(c("icb_", "nhs_region_")),
          paste0("influenza_", config_overall_params$target_name)
        ) |>
        dplyr::filter(
          !is.na(icb_name),
          !is.na(nhs_region_name),
          !is.na(trust_code)
        ) |>
        dplyr::rename_with(\(.) sub("^influenza_", "", .)) |>
  ...
  targets::tar_target(
    ...
    data_raw |>
      dplyr::summarise(
        dplyr::across(
          # implicitly only the target at the moment
          !(date:population),
          \(col) {
            dplyr::pick(date, col) |>
  ...
  targets::tar_target(
    data,
    {
      ...
      cli::cli_warn(
        "Data will be truncated to earliest max date: ",
        "{.val {min(data_max_dates$max_date)}}"
      )
  ...
  targets::tar_target(
    output_path,
    here::here("influenza", "outputs", "admissions", make_timestamp) |>
      fs::dir_create()
  ),
      ...
      plots <- purrr::map(
        ...
        \(model_name) {
          outputs$projections_plotter(
            ...
            y_limit = c("nation" = 1000, "region" = 300),
            x_limit_lower = 112,
  ...
  targets::tar_target(
    plot_projections_nation_region_qa,
    {
      ...
      plots <- purrr::map(
        ...
        \(model_name) {
          outputs$projections_plotter(
            plots_include = list(c("multiple_cis", "lookbacks", "rag")),
            data = result_summary,
            target_name = rlang::sym(config_overall_params$target_name),
            model_name = model_name,
            geography = c("nation", "region"),
  ...
  targets::tar_target(
    plot_projections_icb,
    {
      ...
      region_list <- unique(
        icb_plotting$nhser23nm[!is.na(icb_plotting$nhser23nm)]
      )
  ...
  targets::tar_target(
    output_regional_team_summary,
    {
      ...
      icb_plots <- plot_projections_icb |>
        ...
        projection_plots$tabulate_plot_list(icb, "Influenza")
      ...
      regional_plots <- plot_projections_nation_region[grepl(
        ...
        projection_plots$tabulate_plot_list(region, "Influenza")
