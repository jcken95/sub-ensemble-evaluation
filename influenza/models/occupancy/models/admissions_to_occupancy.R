#' @name admissions_to_occupancy
#' @section Version: 0.0.1
#'
#' @title
#' Influenza occupancy via admissions/discharge posteriors
#'
#' @description
#' Combine admissions and discharge posteriors to create influenza occupancy
#' forecast
#'
#' @details
#' # Development
...
#'
#' @seealso
#' * [admissions_to_occupancy$run_occupancy()]
...
run_occupancy <- function(admissions_data, discharge_data, overall_params) {
  ...
  prepared_admissions <- tidyr::expand_grid(
    ...
    prediction_start_date = unique(stats::na.omit(admissions_data$prediction_start_date)),
    model = unique(stats::na.omit(admissions_data$model)),
  ...
  for (p_start_date in unique(stats::na.omit(prepared_admissions$prediction_start_date))) {

    p_start_date <- as.Date(p_start_date)

    message(glue::glue("Running lookback period {p_start_date}"))
    ...
    for (i_date in seq(p_start_date - max(overall_params$max_lag, overall_params$training_length),
      ...
      window <- subset |>
        ...
        dplyr::mutate(discount = stats::plnorm(j, meanlog = lognormal_mu, sdlog = lognormal_sigma)) |>
        ...
        dplyr::summarise(.pred = stats::rgamma(1, shape = sum(effective) * beta, rate = beta), .groups = "drop") |>
...
#' Combine admissions and discharge posteriors to create influenza occupancy forecast - regional
#'
#' @param admissions_data The loaded in formatted admissions forecast.
#' @param discharge_data The formatted discharge parameters.
#' @param overall_params Hyperparameters of the model.
...
run_occupancy_region <- function(admissions_data, discharge_data, overall_params) {
  ...
  prepared_admissions <- tidyr::expand_grid(
    ...
    date = seq(min(admissions_data$date), max(admissions_data$date), by = "day"),
    prediction_start_date = unique(stats::na.omit(admissions_data$prediction_start_date)),
    location = unique(stats::na.omit(admissions_data$location)),
    model = unique(stats::na.omit(admissions_data$model)),
    .sample = 1:overall_params$n_pi_sample) |>
    # not all prediction start dates need all dates going backward
    dplyr::filter(date >= prediction_start_date - (overall_params$max_lag + overall_params$training_length) &
