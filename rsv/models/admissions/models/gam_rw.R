...
#'
#' @title
#' RSV admissions GAM (random walk) for RSV hospital admissions forecasting.
#'
#' @description
#' Forecasts are produced at the NHS region level for a "full" set of age groups; some age group aggregation can occur
#' in model post processing. The model uses random effects to account for regional and age group effects; fixed effects
#' are used for modelling weekend effects. An offset for age-region population size is also used. No pooling is used.
#'
#' @details
#' # Development
#'
...
#'  \deqn{\log \{E (Y_t) \} = \text{weekend}_t + \text{region} + \text{age group} + W_t + \log (\text{population})}
#'
...
#' * \eqn{\text{age group}} is a random effect for the age group
#' * \eqn{\log (\text{population})} is an offset term based on the size of the region-age group combination
#'
#' # Other documentation
#'
#' * Package docs:
...
#' Run RSV admissions GAM (random walk) model
#'
#' Fit a random walk model to RSV admissions data, and format the outputs.
...
run_gam_rw <- function(
    ...
    output_variables,
    hyperparams) {
  # doesn't work in targets if outside the function
  ...
  model_formula <- stats::as.formula(
    target ~
      weekend_ +
      # k is arbitrary but must be < training_length
      s(
        t_factor,
        bs = "mrf",
