...
#'
#' @title
#' Baseline RSV admissions GAM (day of week effect)
#'
#' @description
#' Generalised additive model (day of week) for RSV admissions forecasting.
#' Forecasts are produced at an the region-age group level with a fixed effect for day of week reporting differences
#' and random effects for NHS region and age group.
#'
#'
#' @details
#' # Development
#'
#' Model first developed at the end of Winter 24/25 to with the aim of providing a baseline model for forecasting RSV
#' hospital admissions.
#'
#'
#' # Key assumptions
#'
#'  A day of week model for forecasting RSV admissions has the following structure:
#'
#'  \deqn{Y_t \sim \text{NegBin}\, (\mu_t, \theta)}
#'  \deqn{\log \{E (Y_t) \} = \text{DoW}_t + \text{region}+ \text{age}}
#'
#' We also define:
#'
...
#'  * \eqn{\text{age}} is a random effect for the age group
#'
#'
#' # Other documentation
#'
...
run_gam_dow <- function(
  ...
  model_formula <- stats::formula(target ~ wday_ + s(nhs_region_name, bs = "re") + s(age_group, bs = "re"))
