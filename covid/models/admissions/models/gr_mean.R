...
#'
#' @title
#' COVID-19 admissions growth rate model (rolling mean target)
#'
#' @description
#' Growth rate model (rolling mean target) for COVID-19 admissions
#' forecasting. Model is fit at the ICB level. Trend is estimated by ICB and region pooled.
#' Model is on a relative scale and therefore there is no population offset.
#'
#' @details
#' # Development
#' Model first developed within winter 2024/25 with the aim of giving the most simple possible
#' epidemic growth rate extrapolation. The model is intentionally simple, with application
#' as a baseline, model of last resort, or to pad out ensembles.
#'
#' # Key assumptions
#' At a high level the model:
#' - calculates a local growth rate based on rolling averages of the target
#' - estimate this local growth rate with a short training length and linear model in time.
...
#'
#'  \deqn{GR_t \sim \mathcal{N}\, (\mu, \sigma)}
#'
#'  \deqn{E (GR) = \text{icb}_\text{region}}
#'
#'  - \eqn{\text{icb}_\text{region}} is therefore a random effect estimating the mean growth rate
#'  over a period of time given to the model, nested by region.
#'
#'  Once this static \eqn{GR} has been estimated we need to convert back to admissions.
#'
...
#'
#'  However, this gives the smoothed admissions, so we can add some \eqn{\text{NegBin}} uncertainty with
#'
#'  \deqn{\text{NegBin}(s(y_t), \theta)}
#'
...
#' Run COVID-19 admissions growth rate (rolling mean target) model
#'
