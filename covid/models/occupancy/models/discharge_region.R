#' @name discharge_region
#' @section Version: 0.0.1
#'
#' @title
#' COVID-19 occupancy via discharge posteriors
#'
#' @description
#' Use discharge posteriors to create COVID-19 occupancy forecast
#'
#' @details
#' # Development
#'
...
#' Use regional discharge posteriors to create COVID-19 occupancy forecast
#'
#' @param training_data The loaded in formatted admissions forecast.
#' @param overall_params Hyperparameters of the model.
#'
#' @returns A list of model output and predictions.
#'
#' @export
run_discharge_region <- function(training_data, overall_params) {
  ...
  regional_training_data_filtered <- regional_training_data |>
    dplyr::filter(
      date >= max(date) - (
        overall_params$max_lag + overall_params$training_length)
    ) |>
  ...
  model <- cmdstanr::cmdstan_model(
    "covid/models/occupancy/models/discharge_region_variation.stan")
  ...
  predictions <- draws |>
    ...
    tidyr::separate_wider_delim(
      cols = "name", names = c("r", "t"), delim = ",") |>
    dplyr::mutate(t = as.integer(t),
      r = as.integer(r)) |>
    dplyr::group_by(t, r) |>
    dplyr::summarise(
      pi_50 = stats::quantile(value, p = 0.5),
      pi_95 = stats::quantile(value, p = 0.95),
      pi_5 = stats::quantile(value, p = 0.05)
    ) |>
    dplyr::ungroup() |>
  ...
  parameters <- draws |>
    tidyr::pivot_longer(
      cols = dplyr::starts_with("lognormal_mu"),
      names_to = "r_lognormal_mu",
      values_to = "lognormal_mu") |>
    dplyr::mutate(
      r_lognormal_mu = as.integer(stringr::str_extract(r_lognormal_mu, "\\d+"))
    ) |>
    tidyr::pivot_longer(
      cols = dplyr::starts_with("beta"),
      names_to = "r_beta",
      values_to = "beta") |>
    dplyr::mutate(r_beta = as.integer(stringr::str_extract(r_beta, "\\d+"))) |>
    # this feels like wasting compute/memory, but not sure of a better way...
    # ... it works
