# COVID-19 occupancy forecast target script
box::use(box / deps_)
...
list(
  ...
  targets::tar_target(
    config_file,
    here::here("covid", "models", "occupancy", "covid_occupancy_config.yaml"),
    format = "file"
  ),
  targets::tar_target(
    config,
  ...
  targets::tar_target(
    admissions_forecast,
    {
      ...
      # TODO: target compatible check on dates here maybe?
    }
  ),
    ...
    command = {
      ...
      rs$REDACTED |>
        ...
        dplyr::select(
          ...
          population,
          total_beds,
          "admissions" = covid_admissions,
          "occupancy" = covid_occupancy
        ) |>
      ...
      # TODO maybe some data check here
    }
  ),
  ...
  targets::tar_target(
    ...
    cue = targets::tar_cue("always"),
    command = {
      box::use(. / covid / models / occupancy / models / discharge_region)
    ...
    los_national_samples,
    {
      expand.grid(
        ...
        dplyr::left_join(
          discharge_results$parameters,
          by = "nhs_region_name",
          relationship = "many-to-many" # hopefully silence warning...
        ) |> # ... move/copy down to next left_join if not.
  ...
  targets::tar_target(
    ...
    cue = targets::tar_cue("always"),
    command = {
      box::use(. / covid / models / occupancy / models / admissions_to_occupancy)
  ...
  targets::tar_target(
    output_path,
    {
      here::here("covid", "outputs", "occupancy", make_timestamp) |>
        fs::dir_create()
    }
  ),
  ...
  targets::tar_target(
    discharge_qa,
    {
      ...
      regional_fit_plot <- discharge_results$predictions |>
        ...
        ggplot2::scale_fill_manual(values = c("occupancy 90% posterior" = "royalblue")) +
        ggplot2::theme(legend.position = "bottom") +
        ggplot2::guides(color = ggplot2::guide_legend(nrow = 3))

      ggplot2::ggsave(regional_fit_plot, width = 10, height = 8, filename = fs::path(path, "regional_fit.png"))
  ...
  targets::tar_target(
    plot_projections_and_rag,
    {
      ...
      outputs$projections_plotter(
        ...
        disease = config$overall_params$disease,
        y_limit = c(2, 2), # bring down if graphs look silly
        should_nudge_x = TRUE
      )
