...
gam_tuning <- tidyr::expand_grid(
  ...
  dplyr::mutate(
    ...
    gam = furrr::future_map(
      gam_formula,
      \(model_spec) try(mgcv::gam(stats::as.formula(model_spec), data = covid_data)),
      .progress = TRUE
    ),
...
posterior_samples <- intervals$generate_samples(covid_data, rps_gam, .n_pi_samples = 10000) |>
...
mean_model_effect |>
  ...
  dplyr::mutate(
    vjust = dplyr::if_else(name == "ets", 0.2, 1),
    hjust = dplyr::if_else(name == "ets", 0.6, 0),
